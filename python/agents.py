...
# Marks a `.claude/agents/*.md` file as generated by AIOS from the Agents
# screen. Lets the materializer prune stale agents WITHOUT touching the
# hand-authored starter-kit subagents (researcher / code-reviewer / brief-writer).
_AIOS_AGENT_MARKER = "<!-- aios-managed: regenerated from the AIOS Agents screen; edits here are overwritten -->"

# Appended to every materialized sub-agent body. The specialist default_prompts
# are written for the sentinel RUNNER (they emit [ASSIGN_TASK]/[AWAITING_APPROVAL]
# etc.); a native sub-agent just returns its result to the orchestrator, so those
# are stripped — but the destructive-action approval gate is a real SAFETY rule,
# kept here in plain language.
_SUBAGENT_GUIDANCE = """

---
You are running as a focused AIOS sub-agent dispatched by the orchestrator. Do your scoped work and return one clear, complete result — don't narrate ("I will now…").
- The user's business context lives in the project's `context/*.md` files and `CLAUDE.md`. Read them when you need background.
- The user's connected services are available through the search + execute tools. Never mention the underlying plumbing in your result.
- SAFETY GATE: before any action that sends, posts, creates, edits, deletes, schedules, or contacts a real person on a real system, do NOT execute blind. Prepare the full action (exact recipients, subject, body, time, amount — no placeholders) and return it as a preview for confirmation first. Read-only work (querying, summarising, drafting) needs no gate.
- Don't fabricate names, numbers, dates, or quotes. If you don't know something, say so."""


def _subagent_body(effective_prompt: str) -> str:
    """Turn a stored agent prompt into a clean native-sub-agent system prompt:
    strip the runner-only protocol blocks + the runtime {USER_CONTEXT}
    placeholder, then append the shared guidance (which preserves the safety
    gate). Custom user prompts (no protocol blocks) pass through unchanged."""
    body = effective_prompt or ""
    body = body.replace(_OUTPUT_PROTOCOL, "").replace(_TOOLS_BLOCK, "")
    body = re.sub(r"USER CONTEXT\s*\n\s*\{USER_CONTEXT\}", "", body)
    body = body.replace("{USER_CONTEXT}", "")
    body = re.sub(r"\n{3,}", "\n\n", body).strip()
    return body + _SUBAGENT_GUIDANCE


def _agent_description(agent: dict[str, Any]) -> str:
    """Frontmatter description — this is what Claude reads to auto-dispatch.
    Single line (YAML), so collapse whitespace."""
    name = agent.get("name") or agent.get("id")
    role = (agent.get("role") or "").strip().rstrip(".")
    base = f"{name} specialist — {role}." if role else f"{name} specialist."
    return re.sub(r"\s+", " ", f"{base} Use proactively when a request fits {name}'s scope.").strip()


def sync_agents_to_claude_dir() -> None:
    """Materialize each AIOS agent (except the CEO orchestrator) as a native
    Claude Code sub-agent at <workspace>/.claude/agents/<id>.md, so the chat
    agent can dispatch to them via the Agent tool with isolated context +
    parallel execution (verified on CLI 2.1.156). Additive: the existing
    overlay/sentinel path is untouched. Never raises — agent sync must not be
    able to break a sidecar boot or an agent mutation."""
    try:
        agents_dir = workspace.workspace_root() / ".claude" / "agents"
    ...
    for agent in list_agents()["agents"]:
        slug = agent["id"]
        if slug == "ceo":
            continue  # the CEO is the orchestrator (the main chat agent), not a dispatch target
        managed_ids.add(slug)
        content = (
            "---\n"
            f"name: {slug}\n"
            f"description: {_agent_description(agent)}\n"
            "---\n"
            f"{_AIOS_AGENT_MARKER}\n\n"
            f"{_subagent_body(agent['effective_prompt'])}\n"
        )
        try:
            (agents_dir / f"{slug}.md").write_text(content, encoding="utf-8")
        except OSError:
            continue
    # Prune AIOS-managed files for agents that no longer exist; leave any file
    # without our marker (the hand-authored starter-kit agents) untouched.
    try:
        for path in agents_dir.glob("*.md"):
            if path.stem in managed_ids:
                continue
            try:
                if _AIOS_AGENT_MARKER in path.read_text(encoding="utf-8"):
                    path.unlink()
            except OSError:
                continue
    except OSError:
